...
async function laadVragen() {
    ...
    fetch(url, {
        ...
        .then(vraag => {
                ...
                console.log(localStorage.getItem("partijid"));
                vraagId = vraag.id;
                if (vraagId == -23){
                    window.location = "toonAntwoordenPartij.html";
                }
...
function volgendeVraag() {
    ...
    window.location = "beantwoordenStellingenPartij.html?vraagid=" + (++vraagid);
...
async function antwoordInvoer(){
    ...
    let res = await fetch('url.json');
    let data = await res.json();
    let url = data.link + "answer/politicalgroup/add/" + localStorage.getItem("partijid");
